...
from .evaluate import EvalClf, EvalReg, ML_METRIC
...
class NeuralNetwork:
    ...
    def multi_layer_perceptron_param(self) -> dict:
        ...
        :return dict:
            Configured Multi-Layer Perceptron (MLP) hyper-parameter set
        """
        return dict(hidden_neurons=np.random.choice(a=geometric_progression()),
...
class NetworkGenerator(NeuralNetwork):
    ...
    def _get_param_space(self) -> dict:
        ...
        """
        return dict(hidden_neurons=np.random.choice(a=geometric_progression(n=2 if len(self.model_param_mutated.keys()) > 0 else 2)),
                    learning_rate=np.random.uniform(low=0.00001, high=0.4),
                    batch_size=np.random.choice(a=geometric_progression(n=8)),
                    sample_size=np.random.choice(a=geometric_progression(n=8)),
